...
for id in `cat deps.txt |sed '/^$/d'|grep -v "#" |sort -u`;do
  ...
  aarch64-$jobid:
    ...
    - name: build
      run: |
        ...
        echo \$MIME
        [ "\$MIME"x == "application/x-gzip"x ] && echo "Already exists" && exit 0 || echo "Not exists"
        sudo apt-get update
  ...
  armv7l-$jobid:
    ...
    - name: build
      run: |
        curl -s -L "https://github.com/arm4rpi/pandoc-deps/releases/download/v0.1/armv7l-$id.tar.gz" -o armv7l-$id.tar.gz
        MIME=\`file -b --mime-type armv7l-$id.tar.gz\`
        echo \$MIME
